// ดึง Model มาใช้งาน (อย่าลืมเช็ค path ให้ตรงกับโฟลเดอร์โปรเจกต์ของกลุ่มคุณนะครับ)
const Class = require('../models/Class');
// const User = require('../models/User'); // ของเพื่อนคนที่ 1
// const Package = require('../models/Package'); // ของเพื่อนคนที่ 2

const seedDatabase = async () => {
    try {
        // ⚠️ หมายเหตุสำคัญ: ก่อนจะรัน Seed ของ Class 
        // เพื่อนคนที่ 1 ต้อง Seed ข้อมูลตาราง User ที่มี role เป็น 'trainer' ให้เสร็จก่อนนะครับ 
        // (ในตัวอย่างนี้ผมสมมติว่ามีเทรนเนอร์ ID: 1, 2, 3, 4 อยู่ในระบบแล้ว)
        ...
        // ใช้ bulkCreate เพื่อเพิ่มข้อมูลทั้งหมดรวดเดียว
        await Class.bulkCreate(classData);
        console.log('✅ สร้างข้อมูล Classes จำนวน 20 รายการ สำเร็จแล้ว!');

    } catch (error) {
        console.error('❌ เกิดข้อผิดพลาดในการ Seed ข้อมูล Classes:', error);
    }
};

// ... (โค้ดรัน seed ของเพื่อนคนอื่นๆ) ...

// module.exports = seedDatabase; // หรือสั่งเรียกฟังก์ชันเลย ขึ้นอยู่กับวิธีที่กลุ่มคุณตกลงกัน